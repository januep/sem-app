...
// Function to load quiz JSON files from the public directory
async function loadQuizModules() {
  const quizDirectory = path.join(process.cwd(), 'public/quizes');
  ...
  try {
    ...
    const modules = filenames
      ...
      .map(filename => {
        ...
        // Create an icon mapping (you can expand this or make it part of the JSON)
        ...
        return {
          ...
          icon: iconMap[path.basename(filename, '.json')] || iconMap['office-safety'] // default icon
        };
      });

    return modules;
  } catch (error) {
    console.error('Error loading quiz modules:', error);
    return [];
  }
}

export default async function ModulesPage() {
  const modules = await loadQuizModules();
